...
def edit(request):
	...
	if request.is_ajax():
		...
		for uploaded_file in request.FILES.getlist('photo'):
			...
			json_response = json_response + "\"" + uploaded_file.name + "\": \"" + destination.name + "\","
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			author = request.user
			article = story(title=title, body=body, uuid=story_id, timestamp=timezone.now(), author=author)
			article.save()
			return render(request, 'blog/index.html', {'posts': [article]})
		else:
			return render(request, 'blog/editor.html', {'post': 'some thing went wrong', 'form': form, 'id': story_id})
	story_id = uuid.uuid4().get_hex()
	form = forms.BlogEditForm()
	return render(request, 'blog/editor.html', {'post': 'edit page', 'form': form, 'id': story_id})
